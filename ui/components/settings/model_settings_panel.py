...
class ModelSettingsPanel(QWidget):
    ...
    def get_model_keys(self):
        try:
            all_config = self.config_service.get_all()
            return list(all_config.get("agent", {}).keys())
        except Exception as e:
            self.show_error("读取配置错误", e)
            return []
    ...
    def save_settings(self):
        try:
            ...
            self.model_combo.addItems(self.get_model_keys())
            if new_model_name in self.get_model_keys():
                self.model_combo.setCurrentText(new_model_name)
            PopupDialog(title="成功", message="模型设置已保存!").exec_()
        except Exception as e:
            self.show_error("保存失败", e)
    ...
    def delete_model(self):
        ...
        dialog = ConfirmDialog(
            title="确认删除",
            message=f"您确定要删除模型: {model_name} 吗？",
            yes_text="删除",
            no_text="取消",
            parent=self
        )
        if dialog.exec_and_get_result():
            try:
                self.config_service.delete_model_config(model_name)
                self.config_service.save()
                self.model_combo.clear()
                self.model_combo.addItems(self.get_model_keys())
                self.update_model_info()
                PopupDialog(title="成功", message=f"模型 {model_name} 已被删除!").exec_()
            except Exception as e:
                self.show_error("删除失败", e)

    def add_model(self):
        try:
            existing_models = self.get_model_keys()
            ...
            self.model_combo.setCurrentText(new_model_name)
            PopupDialog(title="成功", message=f"已新增模型: {new_model_name}").exec_()
        except Exception as e:
            self.show_error("新增模型失败", e)
    ...
    def _load_stylesheet(self):
        ...
        try:
            if os.path.exists(qss_path):
                ...
                QMessageBox.warning(self, "样式加载失败", f"样式文件未找到: {qss_path}")
        except Exception as e:
            self.show_error("加载样式出错", e)
